...
endpointForSampleData ="transaction-data-daily"
sectorForSampleData = "FR_CONSUMER"
now = datetime.today()
...
logging.basicConfig(filename='MDataE-FRConsumer.log',level=logging.DEBUG) 	  	
...
def upload_to_S3Bucket():
     ...
     try:
          ...
          # Get file info
          source_path = os.path.dirname(os.path.abspath("MDataE-FRConsumer.csv"))+'\MDataE-FRConsumer.csv'
...
if __name__ == '__main__':
    ...
    print(endDate)	
    try:
            token = get_token(HOSTNAME,endpointForSampleData, "login id", "password", "False")
            data = get_data(token, HOSTNAME, endpointForSampleData, sectorForSampleData, "","","","", "", "", "", "", "", "", "", "", "", "", "", "", startDate, endDate,"False",None,None)
            convert_data(data, "MDataE-FRConsumer.csv")
            upload_to_S3Bucket()
    except Exception as e:
         logging.debug(e)
